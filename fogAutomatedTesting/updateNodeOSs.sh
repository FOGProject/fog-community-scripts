...
do
    ...
    if [[ "$status" == "-1" ]]; then
        ...
        if [[ -f $webdir/$i/os/${rightNow}.log ]]; then
            ...
            echo "$i failed to update OS, logs here: http://$domainName:20080/fog_distro_check/$i/os/$rightNow.log" >> $report
            echo "$(date +%x_%r) $i failed to update OS, logs here: http://$domainName:20080/fog_distro_check/$i/os/$rightNow.log" >> $output
        else
