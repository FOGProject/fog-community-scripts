...
if [[ $(ssh -o ConnectTimeout=$sshTimeout $hostsystem "virsh domstate $testServerVMName") == "running" ]]; then
    echo "$(date +%x_%r) \"$testServerVMName\" already running." >> $output
else
...
timeout $sshTime scp -o ConnectTimeout=$sshTimeout $cwd/postdownload.sh $testServerSshAlias:/images/postdownloadscripts/postdownload.sh > /dev/null 2>&1
