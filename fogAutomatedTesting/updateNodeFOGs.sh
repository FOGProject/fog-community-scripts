...
do
    ...
    if [[ "$status" == "0" ]]; then
        ...
        if [[ ! -z $foglog || ! -z $commit  ]]; then
            echo "Fog log: http://$domainName:20080/fog_distro_check/$i/fog/${rightNow}_fog.log" >> $report
            echo "$(date +%x_%r) Fog log: http://$domainName:20080/fog_distro_check/$i/fog/${rightNow}_fog.log" >> $output
        else
        ...
        if [[ -f $webdir/$i/fog/${rightNow}_apache.log ]]; then
            echo "Apache log: http://$domainName:20080/fog_distro_check/$i/fog/${rightNow}_apache.log" >> $report
            echo "$(date +%x_%r) Apache log: http://$domainName:20080/fog_distro_check/$i/fog/${rightNow}_apache.log" >> $output
        else
