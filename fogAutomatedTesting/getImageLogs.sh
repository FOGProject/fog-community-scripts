...
if [[ -f ${shareDir}/${testHost}/var-log.tar ]]; then
    ...
    echo "$(date +%x_%r) \"$testHost\" /var/log here: http://${domainName}${port}${netdir}/${testHost}/${rightNow}_var-log.tar" >> $output
    echo "\"$testHost\" /var/log here: http://${domainName}${port}${netdir}/${testHost}/${rightNow}_var-log.tar" >> $report
else
...
if [[ -f ${shareDir}/${testHost}/postinit.log ]]; then
    ...
    echo "$(date +%x_%r) \"$testHost\" postinit.log: http://${domainName}${port}${netdir}/${testHost}/${rightNow}_postinit.log" >> $output
    echo "\"$testHost\" postinit.log: http://${domainName}${port}${netdir}/${testHost}/${rightNow}_postinit.log" >> $report
else
...
if [[ -f ${shareDir}/${$testHost}/postdownload.log ]]; then
    ...
    echo "$(date +%x_%r) \"$testHost\" postdownload.log: http://${domainName}${port}${netdir}/${testHost}/${rightNow}_postdownload.log" >> $output
    echo "\"$testHost\" postdownload.log: http://${domainName}${port}${netdir}/${testHost}/${rightNow}_postdownload.log" >> $report
else
...
if [[ $count -gt 0 ]]; then
    ...
    echo "$(date +%x_%r) \"$testHost\" deploy screenshots: http://${domainName}${port}${netdir}/${testHost}/${rightNow}_deploy_screenshots.tar" >> $output
    echo "\"$testHost\" deploy screenshots: http://${domainName}${port}${netdir}/${testHost}/${rightNow}_deploy_screenshots.tar" >> $report
else
