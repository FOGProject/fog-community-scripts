resource "aws_instance" "almalinux8" {
  ...
  connection {
    type                = "ssh"
    user                = "ec2-user"
    host                = aws_instance.almalinux8[0].private_ip
  ...
  tags = {
    Name    = "${var.project}-almalinux8"
    Project = var.project
    OS      = "almalinux8"
  }
}

resource "aws_route53_record" "almalinux8-dns-record" {
  count   = var.make_instances
  zone_id = aws_route53_zone.private-zone.zone_id
  name    = "almalinux8.fogtesting.cloud"
  type    = "CNAME"
  ttl     = "300"
  records = [aws_instance.almalinux8[0].private_dns]
}
